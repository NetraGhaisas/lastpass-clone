...
if (!isset($_SESSION['is_login'])) {
  if (isset($_REQUEST['rEmail'])) {
    ...
    $rPassword = mysqli_real_escape_string($conn, trim($_REQUEST['hashedPassword']));
    $sql = "SELECT r_email, r_password FROM users WHERE r_email='" . $rEmail . "' limit 1";
    $result = $conn->query($sql);
    if ($result->num_rows == 1) {
      $row = $result->fetch_assoc();
      $serverHashedPassword = $row['r_password'];
      if (password_verify($rPassword, $serverHashedPassword)) {
...
<body>
  ...
  <p class="text-center" style="font-size: 20px;"> <i class="fas fa-user-secret text-danger"></i> <span>User</span>
  </p>
  <div class="container-fluid mb-5">
    <div class="row justify-content-center custom-margin">
      <div class="col-sm-6 col-md-4">
        <form action="" class="shadow-lg p-4" method="POST" id="login" onsubmit="beforeLogin(event)">
          ...
          <div class="form-group">
            <i class="fas fa-key"></i><label for="pass" class="pl-2 font-weight-bold">Master Password</label><input type="password" class="form-control" placeholder="Password" name="rPassword" id="password">
          </div>
